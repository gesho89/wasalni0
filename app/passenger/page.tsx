...
export default function PassengerPage() {
  ...
  return <main className="passenger-app">
    <header className="passenger-header"><button className="icon-button mobile-only" aria-label="القائمة"><Menu size={21}/></button><div className="brand-lockup"><div className="brand-mark"><CarFront size={22}/></div><div><strong>تك توكي</strong><small>TUKTUKY</small></div></div><nav className="passenger-nav"><button className="active"><Home size={16}/>الرئيسية</button><button onClick={() => router.push('/passenger/rides')}><Clock3 size={16}/>رحلاتي</button><button onClick={() => router.push('/passenger/wallet')}><WalletCards size={16}/>المحفظة</button></nav><div className="passenger-actions"><button className="icon-button" aria-label="الإشعارات"><Bell size={19}/><i>3</i></button><button className="passenger-profile" onClick={() => router.push('/passenger/rides')}><span>{passenger.name}</span><div className="avatar"><UserRound size={17}/></div></button></div></header>
