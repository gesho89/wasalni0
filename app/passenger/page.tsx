...
export default function PassengerPage() {
  ...
  return <main className="passenger-app">
    ...
    </section><footer className="passenger-bottom-nav"><button className="active"><Home size={19}/>الرئيسية</button><button onClick={() => router.push('/passenger/rides')}><Clock3 size={19}/>رحلاتي</button><button onClick={() => setRequested(true)} className="center-action"><CarFront size={24}/>اطلب الآن</button><button onClick={() => router.push('/passenger/wallet')}><WalletCards size={19}/>المحفظة</button><button onClick={() => router.push('/passenger/security')}><UserRound size={19}/>حسابي</button></footer>
  </main>
}
