    ...
    <main className="account-page">
      ...
      <section className="account-content">
        ...
        <section className="account-section"><div className="section-label"><KeyRound size={16} /><span>تسجيل الدخول</span></div>
          ...
          <div className="account-row"><span className="row-icon"><Smartphone size={17} /></span><span><b>التحقق بخطوتين</b><small>أضف طبقة حماية إضافية لحسابك</small></span><button className={`toggle ${twoFactor ? "on" : ""}`} onClick={() => setTwoFactor(!twoFactor)} aria-label="التحقق بخطوتين"><i /></button></div>
          <div className="account-row"><span className="row-icon"><UserRound size={17} /></span><span><b>الدخول بالبصمة</b><small>{biometric ? "مفعّل على هذا الجهاز" : "غير مفعّل"}</small></span><button className={`toggle ${biometric ? "on" : ""}`} onClick={() => setBiometric(!biometric)} aria-label="الدخول بالبصمة"><i /></button></div>
        </section>
